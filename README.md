<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=180&color=0:0f766e,50:2563eb,100:7c3aed&text=Hi%20Everyone!&fontAlign=50&fontAlignY=40&fontColor=ffffff&fontSize=48" alt="Eugene Gu profile header" />
</p>

<p align="center">
  <a href="mailto:eugenegujing@outlook.com" title="Email">
    <img width="34" src="./assets/outlook.svg" alt="Email" />
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/eugenegujing" title="GitHub">
    <img width="34" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" alt="GitHub" />
  </a>
</p>

## About

Hi, I'm Eugene Gu. I'm a student at the University of California, Irvine pursuing a B.S. in Computer Science with a specialization in Intelligent Systems, along with a B.S. in Applied and Computational Mathematics with a concentration in Data Science.

I enjoy building practical systems across AI, full-stack development, data engineering, and cloud deployment.

- Based in Irvine, CA
- Native speaker of English and Mandarin
- Open to software engineering, AI/ML, data, and full-stack opportunities

## Technical Toolbox

**Languages**

<p>
  <img src="https://skillicons.dev/icons?i=python,java,cpp,js,mysql,r,matlab&theme=light" alt="Python, Java, C++, JavaScript, SQL, R, and MATLAB" />
</p>

**Frameworks, Data, and ML**

<p>
  <img src="https://skillicons.dev/icons?i=react,angular,sklearn,numpy,pandas&theme=light" alt="React Native, Angular, scikit-learn, NumPy, and pandas" />
</p>

**Backend, Cloud, and DevOps**

<p>
  <img src="https://skillicons.dev/icons?i=aws,docker,kubernetes,linux,bash,git,redis,mysql&theme=light" alt="AWS, Docker, Kubernetes, Linux, Bash, Git, Redis, and MySQL" />
</p>


<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=110&section=footer&color=0:0f766e,50:2563eb,100:7c3aed" alt="Footer wave" />
</p>
